...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <div className="group relative bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-6 hover:shadow-xl dark:hover:shadow-2xl dark:hover:shadow-pink-900/10 hover:-translate-y-1 transition-all duration-300 flex flex-col">
      {/* Category Badge */}
      <div className="flex justify-between items-start mb-4">
        ...
        <div className="text-right">
          {product.note && (
            <span className="block text-xs font-bold tracking-wider text-fuchsia-600 dark:text-fuchsia-400 uppercase mb-1">
              {product.note}
            </span>
          )}
      ...
      <div className="pt-4 border-t border-gray-100 dark:border-gray-800 mt-auto flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          {product.link && (
            <button
              onClick={handleCopy}
              className="p-1.5 text-gray-400 hover:text-pink-600 dark:hover:text-pink-400 transition-colors rounded-md hover:bg-gray-100 dark:hover:bg-gray-800"
              title="Copy Link"
            >
              {copied ? (
